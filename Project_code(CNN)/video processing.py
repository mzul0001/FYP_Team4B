...
def ImageProcessing(img,imgname,flag,label):
    ...
    for (x,y,width,height) in faces:
        ...
        #Tagging emotion
            ...
            img = cv2.putText(img = img, text = label[FaceNo], org =(x, y),fontFace = cv2.FONT_HERSHEY_SIMPLEX ,fontScale =1.0, color =(255, 255, 255), thickness=1)
        FaceNo += 1    
...
def VideoProcessing(videoname):
    ...
    while video.isOpened():
        ...
        frame= ImageProcessing(frame,imgname,0,[]) # detect a face in an image
        writer.write(frame)
        #-----------------
